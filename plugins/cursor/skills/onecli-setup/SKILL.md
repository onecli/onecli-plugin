...
name: onecli-setup
description: >-
  Configure the OneCLI Gateway API key for Cursor. Use when the user asks to set
...
Configure OneCLI once, then start a new Cursor composer session so the
session-start hook can write `~/.onecli/env.sh` and apply gateway exports.
...
new Cursor composer session. The session-start hook fetches gateway exports and
applies session env automatically. If they need to refresh in the current
session, resolve the plugin root as the directory two levels above this
`SKILL.md` and run:
