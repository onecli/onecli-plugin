...
name: onecli-gateway
description: >-
  OneCLI Gateway for Codex: transparent HTTPS proxy instructions for outbound
...
For supported outbound Bash commands, the Codex `PreToolUse` hook will
conservatively auto-source the loader. If a command is not auto-sourced and
`HTTPS_PROXY` is not already set to a OneCLI proxy, prefix it with:
